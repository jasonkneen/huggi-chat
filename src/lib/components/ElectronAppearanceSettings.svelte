<script lang="ts">
  ...
  onMount(async () => {
    if (typeof window !== 'undefined' && (window as any).electronAPI) {
      isElectron = true
      isMac = (window as any).electronAPI.platform === 'darwin'

      // Load current settings
      const currentSettings = await (window as any).electronAPI.getAppearanceSettings()
      if (currentSettings) {
  ...
  async function updateVibrancy(event: Event) {
    ...
    const result = await (window as any).electronAPI.setVibrancy(type)
    if (result.success) {
  ...
  async function updateOpacity(event: Event) {
    ...
    const result = await (window as any).electronAPI.setOpacity(opacity)
    if (result.success) {
  ...
  async function updateBlur(event: Event) {
    ...
    await (window as any).electronAPI.setBlur({ blur, saturation: settings.saturation })
    settings.blur = blur
  }
  ...
  async function updateSaturation(event: Event) {
    ...
    await (window as any).electronAPI.setBlur({ blur: settings.blur, saturation })
    settings.saturation = saturation
  }
</script>

{#if isElectron}
  <div class="space-y-6 rounded-xl border border-gray-200 bg-white/50 p-6 backdrop-blur-sm dark:border-gray-700 dark:bg-gray-800/50">
    <div>
